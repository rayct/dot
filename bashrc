...
export LANG=en_GB.utf8 # assuming apt install language-pack-en done
...
shopt -s checkwinsize expand_aliases globstar dotglob extglob
#shopt -s histappend
set -o vi
...
# alias rwxrob="twitch-view rwxrob"
...
# Ensure tmux configuration is sourced
#if [ -n "$TMUX" ]; then
#    echo "Sourcing .tmux.conf from .bashrc"
#    tmux source-file ~/.tmux.conf
#fi

# Ensure tmux configuration is sourced
if [ -n "$TMUX" ] && [ -z "$TMUX_CONF_LOADED" ]; then
    #echo "Sourcing .tmux.conf from .bashrc"
    tmux source-file ~/.tmux.conf
    export TMUX_CONF_LOADED=1
fi
